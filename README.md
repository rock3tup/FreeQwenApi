...
### 3.1 Ручная настройка аккаунта (для сервера)

Если вы запускаете прокси на сервере без графического интерфейса (только командная строка), вы не сможете использовать интерактивное меню для добавления аккаунта, так как оно требует входа через браузер.

В этом случае вы можете настроить аккаунт вручную:

1.  **Получите токен на локальном компьютере:**
    *   Откройте браузер и зайдите на сайт [https://chat.qwen.ai/](https://chat.qwen.ai/).
    *   Войдите в свой аккаунт.
    *   Откройте инструменты разработчика (клавиша F12).
    *   Перейдите на вкладку "Application" (или "Storage").
    *   В меню слева выберите "Local Storage" -> `https://chat.qwen.ai`.
    *   Найдите ключ `token` и полностью скопируйте его длинное значение.

2.  **Настройте аккаунт на сервере:**
    *   Убедитесь, что директория `session/` существует в корне проекта. Если нет, создайте ее.
    *   Откройте или создайте файл `session/tokens.json`.
    *   Добавьте в этот JSON-файл объект для вашего нового аккаунта, используя скопированный токен. Файл должен выглядеть так:

    ```json
    [
      {
        "id": "acc_your_custom_id",
        "token": "СЮДА_ВСТАВЬТЕ_ВАШ_ТОКЕН",
        "resetAt": null,
        "invalid": false
      }
    ]
    ```
    *   Замените `"СЮДА_ВСТАВЬТЕ_ВАШ_ТОКЕН"` на ваш реальный токен.
    *   Вы можете добавить несколько аккаунтов, перечислив их в массиве через запятую.

После сохранения файла `session/tokens.json` прокси сможет использовать ваш аккаунт для работы.

### 3.2 Настройка автозапуска с помощью systemd (Linux)

Чтобы ваше приложение автоматически запускалось после перезагрузки или сбоя на сервере с `systemd` (стандарт для большинства современных дистрибутивов Linux, включая Ubuntu), вы можете создать службу.

#### 3.2.1 Флаг `--autostart`

Для работы в качестве службы был добавлен флаг `--autostart`. При запуске с этим флагом (`npm start -- --autostart`) приложение пропускает интерактивное меню и сразу запускает прокси, если найден хотя бы один валидный аккаунт.

#### 3.2.2 Создание службы systemd

1.  **Создайте файл службы:**
    ```bash
    sudo nano /etc/systemd/system/qwen-proxy.service
    ```

2.  **Вставьте в файл следующее содержимое:**
    ```ini
    [Unit]
    Description=Qwen AI API Proxy Server
    ...
    # Укажите ПОЛНЫЙ путь к директории вашего проекта
    WorkingDirectory=/path/to/your/project/FreeQwenApi

    # Укажите ПОЛНЫЙ путь к npm и команду запуска с флагом --autostart
    ExecStart=/usr/bin/npm start -- --autostart
...
3.  **Настройте шаблон:**
    *   Замените `your_username` на ваше имя пользователя (узнать можно командой `whoami`).
    *   Замените `/path/to/your/project/FreeQwenApi` на полный путь к вашему проекту (узнать можно командой `pwd` внутри папки проекта).
    *   Убедитесь, что путь к `npm` верный (проверить можно командой `which npm`).
...
4.  **Активируйте и запустите службу:**
    ...
    sudo systemctl enable qwen-proxy.service
    # Запустить службу немедленно
    sudo systemctl start qwen-proxy.service
    ```

5.  **Проверка статуса и логов:**
    ```bash
    # Проверить статус
    sudo systemctl status qwen-proxy.service
    # Смотреть логи в реальном времени
    journalctl -u qwen-proxy.service -f
    ```
